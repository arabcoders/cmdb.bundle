    ...
    try:
        ...
        for rx in data:
            ...
            try:
                ...
                Log.Error("Error compiling custom regex: " + str(rx) + " - " + str(e))
    except Exception as e:
...
def Start():
    ...
    HTTP.Headers["User-Agent"] = (
        "Mozilla/5.0 (iPad; CPU OS 7_0_4 like Mac OS X) AppleWebKit/537.51.1 (KHTML, like Gecko) Version/7.0 Mobile/11B554a Safari/9537.54"
    )
...
class CustomMetadataDBSeries(Agent.TV_Shows):
    ...
    def getShowInfo(self, media):
        ...
        if not filename:
            Log("getShowInfo() - No file found in media object.")
            return None
        ...
        Log(
            "getShowInfo() - filename: {} dirName: {} parentDirName: {}".format(
                filename, dirName, parentDirName
            )
        )
        ...
        try:
            ...
            if not os.path.exists(nfoFile):
                Log("getShowInfo() - No tvshow.nfo file found in: {}.".format(dirName))
                return None
            ...
            Log("getShowInfo() - info: {}".format(info))

            return info
        except Exception as e:
            Log("getShowInfo() Exception: {}".format(e))
            Log("getShowInfo() Traceback: {}".format(traceback.format_exc()))
            return None
    ...
    def getEpisodeNFO(self, filename):
        ...
        try:
            ...
            if not os.path.exists(nfoFile):
                Log("getEpisodeNFO() - No NFO file exists for: {}.".format(filename))
                return {}
            ...
            for key in keys:
                if isinstance(key, tuple):
                    xml_key, info_key = key
                else:
                    xml_key = key
                    info_key = key

                try:
                    value = nfoXML.xpath(xml_key)[0].text.strip()
                    if value:
                        info[info_key] = value
                except:
                    continue

            Log("getEpisodeNFO() - info: {}".format(info))
            return info
        except Exception as e:
            Log("getEpisodeNFO() Exception: {}".format(e))
    ...
    def search(self, results, media, lang, manual):
        ...
        if not json:
            Log("Search() - No results found for: [{}]".format(media.show))
            return

        for item in json:
            Log(
                "Search() - found - id: {}, title: {}".format(item["id"], item["title"])
            )
            results.Append(
                MetadataSearchResult(
                    id=item["id"], name=item["title"], lang=lang, score=100
                )
            )
                ...
                for e in sorted(media.seasons[s].episodes, key=natural_sort_key):
                    ...
                    def updateEpisode(episode=episode, realFile=episode_file, metadata=metadata):
                        ...
                        for rx in RX_LIST:
                            ...
                            data = self.handleMatch(match, metadata.title, realFile)
                            if not data:
                                continue
                            ...
                            if data.get("released_date"):
                                episode.originally_available_at = Datetime.ParseDate(data.get("released_date")).date()
                                if metadata.originally_available_at is None:
                                    metadata.originally_available_at = episode.originally_available_at

                            found = True
                            Log(
                                "updateEpisode() - season: {}, episode: {}, title: {}, summary: {}, released_date: {}, file: {}".format(
                                    s,
                                    episode.index,
                                    episode.title,
                                    episode.summary,
    ...
    def handleMatch(self, match, show, file=None):
        ...
        episode = (
            match.group("episode") if match.groupdict().has_key("episode") else None
        )
        ...
        released_date = (
            "%s-%s-%s" % (year, month, day) if year and month and day else None
        )
        ...
        if season is None and nfo.get("season"):
            season = int(nfo.get("season"))

        if episode is None and nfo.get("episode"):
            episode = int(nfo.get("episode"))
    ...
    def searchCustomDB(self, type, query):
        ...
        Log("_searchCustomDB() - path: {}, key: {}".format(apiUrl, query))
